...
```shell
wget https://mirror.ghproxy.com/https://raw.githubusercontent.com/zh15933/pve/main/pve.sh -O /usr/bin/pve && chmod +x /usr/bin/pve
```
...
```shell
bash -c  "$(curl -fsSL https://mirror.ghproxy.com/https://raw.githubusercontent.com/zh15933/pve/main/pve.sh)"
```
...
```shell
wget https://mirror.ghproxy.com/https://raw.githubusercontent.com/zh15933/pve/main/openwrt.lxc.sh -O /usr/bin/openwrt && chmod +x /usr/bin/openwrt
```
...
```shell
bash -c  "$(curl -fsSL https://mirror.ghproxy.com/https://raw.githubusercontent.com/zh15933/pve/main/openwrt.lxc.sh)"
```
...
| 选项     | 说明                                                         | 默认值              |
| -------- | ------------------------------------------------------------ | ------------------- |
| 仓库地址 | github用户名称/仓库名称                                        | zh15933/OP-LXC |
...
| 选项     | 说明                                   | 默认值  |
| -------- | -------------------------------------- | ------- |
| 容器ID   | lxc容器id，需要>=100                   | 110     |
| 容器名称 | lxc容器管理页面显示的名称              | OpenWrt |
| CPU核心  | lxc容器分配CPU核心数                   | 2|
| 内存大小 | lxc容器分配内存大小，单位MB            | 1024    |
| 磁盘大小 | lxc容器分配磁盘大小，单位GB            | 1       |
